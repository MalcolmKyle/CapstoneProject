<?php
    ...
    // Get list of haves that a user has posted
    $havesQuery ="SELECT item_name, category, description, Items.item_id
                  FROM Items INNER JOIN Has INNER JOIN Users
                  ON Items.item_id = Has.item_id
                  WHERE Users.username = '$username'
                  AND Has.user_name = '$username' ";

    $havesResult = $connection->query($havesQuery);
    if (!$havesResult) die($connection->error);
...
<html lang="en">
    ...
    <br>
    <br>
    <h2>View Personal Haves</h2>
      <?php 
          if ($havesResult->num_rows >0 )
          {
      ?>
      <table class="table table-hover table-bordered table-stripped">
        <thead><tr><th>Name</th><th>Category</th><th>Description</th><th>Offers</th></tr></thead>
        <tbody>
          <?php
            // output data as table
            while ($row=$havesResult->fetch_array())
              {?>
                <tr>
                  ...
                  <td><a href="viewOffers.php">
                        <?php 
                           // Has offers list
                          $offersQuery = "SELECT *
                                          FROM Offers NATURAL JOIN Participates
                                          WHERE Participates.receiver = '$username'
